...
export default function CouponSection({ handleBooking, setOrderData }: { handleBooking: any, setOrderData: any }) {
    ...
    const handleApply = async (code: string) => {
        ...
        try {
            ...
            if (bookingDataStr) {
                const bookingData = JSON.parse(bookingDataStr);
                const response: any = await Post("/api/booking/apply", { bookingId: bookingData._id, couponCode: code }, 5000, true);
                if (response?.success) {
                    setIsAnimating(true);
                    setTimeout(() => {
                                ...
                                return (
                                    ...
                                    >
                                        <div className="p-4 flex flex-col h-full cursor-pointer items-start justify-between">
                                            ...
                                            <p className="text-gray-500 pt-2 mb-2 text-sm line-clamp-2">{coupon.description}</p>
